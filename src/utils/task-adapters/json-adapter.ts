 ...
 * JSON Task Adapter
 * 
 * Default adapter that stores tasks as JSON files in ~/.pi/teams/<team>/tasks/
 */
...
function getTaskId(teamName: string): string {
...
export class JsonTaskAdapter implements TaskAdapter {
  readonly name = "json";

  detect(): boolean {
    // JSON adapter is always available as fallback
    return true;
  }
  ...
  async delete(teamName: string, taskId: string): Promise<void> {
    await this.update(teamName, taskId, { status: "deleted" });
  }
  ...
  async submitPlan(teamName: string, taskId: string, plan: string): Promise<TaskFile> {
    ...
    return await this.update(teamName, taskId, { status: "planning", plan });
  }
  ...
  async log(teamName: string, taskId: string, message: string): Promise<void> {
    // JSON adapter doesn't support structured logging
  }
  ...
  async handoff(
    ...
    // JSON adapter doesn't support structured handoffs
  }
}
